...
const Header: React.FC<{ setBg: boolean }> = ({ setBg = false }) => {
  ...
  return (
    <motion.header
      ...
      className={`w-full overflow-hidden min-h-[83px] transition-colors duration-500 relative top-0 left-0 z-40 ${
    ...
    >
      <nav className="ssm:mx-[10%] xl:mx-[20%] relative flex items-center px-4 py-2 justify-between ssm:flex-col lg:flex-row">
        <div className="flex items-center gap-4 xl:justify-between">
          <div className="flex items-center justify-between ">
            <img
              ...
              alt="Logo"
              width={150}
              className={`ssm:absolute ssm:-left-10 ssm:top-2 ssm:ml-0 ssm:mt-0xl:static xl:ml-0 xl:mt-0`}
              style={{ zIndex: 30 }}
            />
            <div
        ...
        <div className={`hidden xl:block xl:z-10 `}>
          <div className="flex ">
            <ul
              className={`flex items-center gap-4 ssm:flex-col lg:flex-row mr-32`}
            >
              <li className="group">
                <a href="#home" className={styles.text}>
                  Home
